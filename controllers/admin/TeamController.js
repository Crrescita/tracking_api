...
exports.createBranch = async (req, res, next) => {
  try {
    ...
    if (id) {
      const branchRecord = await sqlModel.select("branch", ["name"], {
        id,
      });
      if (branchRecord.error || branchRecord.length === 0) {
        return res
          .status(200)
          .send({ status: false, message: "Branch not found" });
      }

      insert.updated_at = getCurrentDateTime();
      const saveData = await sqlModel.update("branch", insert, { id });
      ...
      const existingSlug = await sqlModel.select("branch", ["id"], {
        slug,
        company_id,
      });
      ...
      insert.created_at = getCurrentDateTime();
      const saveData = await sqlModel.insert("branch", insert);
...
exports.getBranch = async (req, res, next) => {
  try {
    ...
    const data = await sqlModel.select("branch", {}, whereClause);
...
exports.deleteBranch = async (req, res, next) => {
  try {
    ...
    const branchRecord = await sqlModel.select("branch", {}, { id });

    if (branchRecord.error || branchRecord.length === 0) {
      return res.status(200).send({ status: false, message: "Data not found" });
    }

    let result = await sqlModel.delete("branch", { id: id });
...
exports.deleteMultipleBranch = async (req, res, next) => {
  try {
    const ids = req.body.ids;
    ...
    const results = await Promise.all(
      ids.map((id) => sqlModel.delete("branch", { id }))
    );
