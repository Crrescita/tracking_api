...
exports.getCoordinates = async (req, res, next) => {
  try {
    let query =
      "SELECT emp_id,company_id,date,battery_status,gps_status,internet_status,motion,created_at,updated_at, time(datetime_mobile) as time, datetime_mobile, round(latitude,6) as latitude, round(longitude,6) as longitude FROM emp_tracking WHERE latitude != 0.0 AND longitude != 0.0";
